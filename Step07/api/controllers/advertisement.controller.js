...
exports.create = (req, res) => {
    ...
    User.findCompanie(req.params.userId, (err, companieId) => {
        if (err) {
            if (err.kind === "not_found") {
                res.status(404).send({
                    message: `Not found companie with id user ${req.params.userId}.`
                });
            } else {
                res.status(500).send({
                    message: "Error retrieving companie with id user " + req.params.userId
                });
            }
        } else {
            console.log("id companie");
            console.log(companieId[0].id);
            ...
            const advertisement = new Advertisement({
                ...
                fk_entreprise_id: companieId[0].id,
            });
